...
if [[ $1 == 'release' ]]; then
  ...
  TEMP_SITE_PATH=$TEMP_PATH/marknotes-$RELEASE_TAG
  if [[ -e $TEMP_SITE_PATH ]]; then
    rm -rf $TEMP_SITE_PATH
  fi
  git archive -o $TEMP_PATH/marknotes-$RELEASE_TAG.zip $RELEASE_TAG
  mkdir -p $TEMP_SITE_PATH
  pushd $TEMP_SITE_PATH
  unzip ../marknotes-$RELEASE_TAG.zip
  popd
fi

SITE_NAME=$1
SITE_PATH=$SCRIPT_DIR/sites/$SITE_NAME
export MARKNOTES_CONFIG=$SITE_PATH/.marknotes.json
